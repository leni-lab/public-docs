...
extension, matched case-insensitively, from its filename to obtain the
namespace. The namespace must be nonempty and have no surrounding whitespace:
...
Namespaces, heading paths, field names, and directive keywords are compared
without case sensitivity. Comparison trims syntax padding and converts names
to lowercase. It does not collapse internal whitespace, remove accents, or
replace spaces with underscores in heading names. Normal output and display
use normalized names. Original spelling is retained only for diagnostics.
Field values retain their case and internal whitespace except when processed
by an additive list operation.
...
updating absolute references to it. Local references remain page-relative.
...
with an ASCII letter and contains only ASCII letters, digits, `_`, and `-`.
Its spelling is compared without case sensitivity and exposed in lowercase.

Whitespace around the separating colon is syntax padding. The name is the
entire remaining text, with outer whitespace removed and converted to
lowercase. It must not be empty. Further colons belong to the name. Original
spelling is retained internally for diagnostics.
...
| `User:Alice`              | `user`    | `alice`                  |
| `USER : Alice`            | `user`    | `alice`                  |
| `abo: Customer: North`    | `abo`     | `customer: north`        |
| `templates`              | none      | `templates`              |
| `Delivery notes: Mail`   | none      | `delivery notes: mail`   |
...
directive names and cannot be field names.
...
Each directive occupies a complete line and names exactly one target.
...
colon is the path, so spaces and commas in titles are not list separators.
...
A local reference always starts at the page root, not the current heading.
Absolute references start with `/`, followed by a namespace and heading path.
Path components are nonempty. Leading or trailing component whitespace,
trailing separators, and relative-navigation components are invalid.
References target headings, not page roots. Lookup is case-insensitive and
never falls back to a search in other pages.
...
type, name, child nodes, prose, or record identity. Nested local references
...
2. lowercase the page title and replace ASCII spaces with underscores
3. append `.wiki`
...
errors rather than silently ignored prose. `include` and `use` are reserved
