...
[appinfo]
patterns = C:/apps/**/appinfo.ini, D:/config/*/appinfo.ini
...
`[appinfo] patterns` is a comma-separated list of file search patterns. Relative
...
read once. Missing
roots have no matches; access errors stop loading with a diagnostic. If the
combined search finds no files, startup fails. Exclusions are not supported.
...
Apps without a group appear first, without a group heading. Named groups follow
in the order they are first encountered, with apps in discovery order within
each group. Surrounding whitespace is removed from group names. Equal names
are combined across all manifests, regardless of profile. Group names are
