...
For Storeindex use `leniextract --field headline_raw,src_id,src_rev,keywords_raw,service,channels,dispatch_at`.
In Storeindex, map `headline = headline_raw`, `keyword = keywords_raw`,
and `channel = channels` in `[fields]`. Other names pass through unchanged.
