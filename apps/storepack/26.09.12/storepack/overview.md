...
for example `1789306733_001.leni.json`. The leading timestamp is `released_at`,
the source release for distribution defined by that contract. Storepack
preserves it when packing or restoring files. Storepack reads only the leading
...
Storepack does not validate source names or release identity.
