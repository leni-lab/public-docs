...
The host and worker use `toolkit.processor/1-draft`. Toolkit owns framing,
readiness, byte limits, process-tree cleanup, and exit validation. Its
[transport contract](https://github.com/leni-lab/py-cli-toolkit/blob/main/processor-contract.md)
is authoritative. configflow has no dependency on the transport implementation.

| part            | contract                                               |
| --------------- | ------------------------------------------------------ |
| request payload | complete JSON input document, UTF-8 without BOM         |
| success payload | complete INI set encoded with Toolkit file-set codec v1 |
| report          | optional `text/markdown` bytes, UTF-8 without BOM        |
| stderr          | technical diagnostics, retained under a byte limit      |
| rejection       | no INI payload, optional Markdown error report          |

Each job configures an exact operation name and version independently of the
worker. Readiness must match both. The operation specification binds this
input/output profile to the domain's validation and normalization rules.
The usermgr operation is `wiki2config-usermgr.compile`, version `1-draft`.

The host supplies `--processor` for one request per process. Workers also
support `--serve` for serial requests until EOF. stdin and stdout are binary
protocol channels, never bare JSON input or Markdown output. Help and version
remain ordinary CLI operations. No banners or log messages may enter stdout.
The former `--output` directory interface is not supported. Upgrade each host
and worker together, with no automatic fallback to the old protocol.

The processor reads no source files or previous output and writes no candidate
files. It returns bytes only. It must not access the active output directory.
Its report describes conversion, not successful activation or archival.

Expected input/domain errors use `invalid_input` rejection with a Markdown
report. Unsupported options and resource bounds use the transport's defined
rejection codes. Unexpected failures remain process failures with stderr
diagnostics. A rejected, malformed, timed-out, or interrupted conversion never
provides an activatable candidate. Toolkit's one-shot host validates the
complete response, clean EOF, and matching exit status before acceptance.
...
The processor owns domain validity and completeness. The host validates logical
filenames for the destination filesystem, including Windows reserved device
names, before staging any received bytes. In particular, the usermgr
...
| -------------- | ------------------------------------------------------------------ |
| file set       | at least one named `.ini` entry without directory components |
| other entries  | no subdirectories, temporary files, or other output types   |
...
A successful response with an empty file set is an error. A logically empty
