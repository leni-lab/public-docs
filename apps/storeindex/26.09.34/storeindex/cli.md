...
| ------------------------------- | ------------------------------------------------------------------------ |
| no command                      | same as `fill`, one pass over loose news items                           |
...
or `--since YYYY-MM-DD` to include loose files and ZIPs from a publication date.
...
Place `--watch` after `fill`. The former top-level `--watch` option is no longer
accepted. A commandless invocation remains equivalent to a single `fill` pass.
...
During startup, a running extractor version check finishes or reaches
...
local system timezone and uses the publication timestamp, not the import time.
...
Existing entries are identified by source, publication timestamp, and suffix,
...
# resume a run limited to a publication date
storeindex fill --since 2026-09-01
```
...
| `-v` (DEBUG)            | processing pending files, maintenance processing, scan and cache counts, lock deferrals, and summaries with elapsed time |
| `-vv` or `-vvv` (TRACE) | skipped file decisions and extraction details                                                                            |
| `-q`                    | warnings and errors                                                                                                      |

Each `indexed` message includes the filename and its publication timestamp
decoded in the local system timezone. This is the publication time, not the
...
summaries at the default level. During incremental processing, rejected files
...
includes every cache hit. Extractor versions are shown per source at startup and
do not cause automatic reprocessing.
...
In session mode, TRACE reports startup-to-readiness once and each request's
...
extractor configuration in [settings](settings.md#persistent-extractor-sessions).
