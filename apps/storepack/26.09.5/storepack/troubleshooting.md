...
The scan summary reports direct entries, entries excluded by `match`,
matching directories, and valid files whose day is still too young. The
internal lock file is excluded from these counts. `ignored` in the final
summary counts matching unsupported files, not files that are too young.

Use `--verbose` to show the resolved store path, filename filter, minimum age,
and the earliest eligibility time among the files that are still too young.
The eligibility time uses the local timezone and includes its UTC offset.
...
00:00 local time. Before then, `0 selected` is expected.
