...
| `source <name>.contract` | required | expected operation name |
| `source <name>.version` | required | exact operation version |
...
Every distinct command and expected contract is checked at Toolkit readiness
before database changes. A failed check stops the entire run, including when
that source has no pending items. `--version` remains a human-facing command.
Remove `--serve` and `--processor` from configured commands, because Toolkit
adds the appropriate flag. Old process protocols are no longer supported.
The existing database needs no migration or rebuild for this transport change.
...
Both modes require complete Toolkit responses. A complete input rejection
allows processing to continue, with quarantine only during incremental work.
Protocol failures, invalid options, incompatible contracts and unexpected
reports stop the run without quarantining the source. Exit code 3 alone does
not establish a rejection. Use the updated Leniextract with this Storeindex.
...
process per document. Session mode retains one process per command and contract.
...
`index.timeout` applies separately to readiness and each complete exchange.
...
The default limits are 64 MiB per request, including the application parameter
line, 128 MiB per result and 4 MiB per report. This application accepts no reports.
