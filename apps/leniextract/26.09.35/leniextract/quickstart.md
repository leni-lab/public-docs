...
```text
leniextract --field headline_raw --input 1789203039_001.leni.json
```
...
```text
leniextract --field headline_raw --input -
```
...
If the archive includes a `leniextract_lib` folder, keep it beside the EXE.
This folder variant avoids unpacking the program at every start.

Extract a title from a UTF-8 file:

```powershell
./leniextract.exe --field headline_raw --input ./message.leni.json
```
