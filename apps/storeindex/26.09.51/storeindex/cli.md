...
or `--since YYYY-MM-DD` to include loose files and ZIPs from a release date.
...
local system timezone and uses the release timestamp, not the import time.
...
Existing entries are identified by source, release timestamp, and suffix,
...
# resume a run limited to a release date
storeindex fill --since 2026-09-01
```
...
Each `indexed` message includes the filename and its release timestamp
decoded in the local system timezone. This is the release time, not the
