...
--	changed definitions take effect with storeindex rebuild
...
CREATE TABLE IF NOT EXISTS item (
	...
	transmit_at 	INTEGER		NOT NULL 			-- transmit_at with fallback to update_at
								CHECK (transmit_at >= 0),
...
CREATE INDEX IF NOT EXISTS item_transmit_at ON item(transmit_at);
...
CREATE TABLE IF NOT EXISTS item_keyword (
	...
	keyword INTEGER NOT NULL REFERENCES keyword(id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS item_channel (
	item INTEGER NOT NULL REFERENCES item(id) ON DELETE CASCADE,
	channel INTEGER NOT NULL REFERENCES channel(id) ON DELETE RESTRICT,
	position INTEGER NOT NULL CHECK (position >= 0),
	PRIMARY KEY (item, position),
	UNIQUE (item, channel)
);
...
--	creating this table does not index existing rows, use rebuild

CREATE VIRTUAL TABLE IF NOT EXISTS item_fts
	USING fts5(
		title,															 -- searchable headline from item.title
...
BEGIN
	INSERT INTO item_fts(rowid, title) VALUES (new.id, new.title);
END;
...
BEGIN
	INSERT INTO item_fts(item_fts, rowid, title)
