...
One writer holds `.storepack.lock` for its complete run, including pauses in
watch mode. Stop the other writer first. The lock file remains after shutdown
...
problem and run again. Watch mode retries on later scans.
