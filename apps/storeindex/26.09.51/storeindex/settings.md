...
`item.released_at` comes from the store filename and is the only time used
for chronological queries. It follows the [newsstore release-time contract](https://github.com/leni-lab/newsstore/blob/main/docs/store-contract.md#release-time):
the source has released this version for distribution to subscribers. It
does not indicate store arrival, delivery, or permission for public publication.
It contains nonnegative integer Unix seconds and
stays unchanged when the index is rebuilt. Do not request it from Leniextract
or assign an extracted field to it, because it is reserved store metadata.

Payload `transmit_at` and `update_at` are not required or validated for this
index. Historical imports may use an approximate release time in their
filenames, as defined by their producer.

The former `published_at` column is now named `released_at`, and
`dispatch_at` has been removed. Remove `dispatch_at` from existing extractor
commands, deploy the matching Storeview version, and run a full `rebuild`
with the new SQL schema. Initialization does not migrate an existing database.
Existing store filenames and JSON contents remain unchanged.
...
News items use `<released_at>_<suffix>.<source>.json`, for example:

```text
1789203039_001.leni.json
```

The release time `released_at` is whole Unix seconds without leading zeros.
