...
Check the final error and log first, then verify the selected configuration,
input paths, and filenames. After resolving the cause, run `--dry-run` before
publishing again.
...
With the example configuration, use `-v` and check the log file for
unchanged-page diagnostics. See [logging](settings.md#logging) for its location
and for enabling DEBUG messages on standard output.
