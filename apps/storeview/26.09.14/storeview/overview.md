...
storeview is a read-only news reader for a storeindex database. It brings
transmission times, services, channels, keywords, locations, and titles into one compact
browser view.
...
The interface is English. Future transmission times appear red and italic.
Times use the server's local time, which may differ from the reader's time.

The current version provides a list only. There is no detail view or full-text
news display, and the list does not refresh automatically.

## Getting started

Ask your operator for the storeview address and open it in a browser. Start
