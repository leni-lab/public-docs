...
The final summary counts eligible files, completed files, and failed files:

```text
store: 12 matching files, 10 done, 2 failed
```

`matching files` includes only valid files that match the filename filter and
are old enough. `done` counts files in successfully completed days. If a day
fails, all its selected files count as `failed`, including any originals
already removed before a cleanup failure. Dry runs report `checked` instead
of `done`. Unsupported files remain individual warnings.

`--verbose` shows the resolved store path, filter, and exclusive local date
boundary. `-vv` also shows scan counters and the next eligibility time. See
