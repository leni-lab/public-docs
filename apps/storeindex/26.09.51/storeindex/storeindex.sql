...
CREATE TABLE IF NOT EXISTS item (
	...
	released_at	INTEGER		NOT NULL 			-- epoch time from filename (*)
								CHECK (released_at >= 0),
	...
	UNIQUE (source, released_at, suffix)			-- news item identity (*)
);

--------------------------------------------------------------------------------
--	application-specific indexes, extend as needed
--	release times support chronological queries and rebuild cutoffs

CREATE INDEX IF NOT EXISTS item_released_at ON item(released_at);
