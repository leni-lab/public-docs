...
timestamp, service, channels, keywords, locations, and title. A source column also
appears when several sources are available.

Items use the store order `(timestamp, suffix, source)`, newest first, with
numeric timestamp and suffix comparison. The **timestamp** column shows the
permanent time component of the store position. It may be a logical value
oriented to the clock and does not establish source release or exact import
time. Times use the server's local time.

A page contains about 50 items. Items with the same timestamp stay
...
Channels use the same labels as the list. Keyword and location expressions
share one gray surface per field, with spaces around each ampersand and one
after each comma.
Timestamp stays on one line. Empty filters keep a light outline. Active
...
| field        | input                                              |
| ------------ | -------------------------------------------------- |
| timestamp         | newest allowed time; short forms also accepted     |
...
| locations    | exact dateline places combined with `,` or `&`      |
...
Keyword and location matches are exact and case-sensitive. Use a comma for OR
and only `&` for AND. Plus signs are literal, so `RTL+` is one keyword.
There is no quoting or escaping. Locations are places from the source dateline,
not every place mentioned in the story or the service locality text.
The two filters combine with AND, like all column filters.
...
Both fields use the same completion behavior, with separate frequency counts.
Type the beginning of a keyword or location to see up to ten suggestions, ordered by
...
Keywords or locations containing a comma or ampersand are displayed in the list,
...
Unavailable controls are gray. Paging fills the timestamp field with the
...
timestamp field's time, rather than only the visible page. The count therefore decreases as you
browse older items.

Clearing timestamp returns to the newest matching items. Reload the page to
...
out of the address. The timestamp limit uses the `until` parameter.
Lists and keyword operators have no surrounding spaces in the address, for
example `service=bas,lnb&keywords=Berlin%26Politik&until=2026-01`.
...
Existing bookmarks that used `+` for AND must use `&` instead (`%26` in URLs).
A literal plus is encoded as `%2B`. The application encodes both automatically.
The title field continues to use its documented FTS5 syntax.
