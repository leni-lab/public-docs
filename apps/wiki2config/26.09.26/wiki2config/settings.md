...
| `output_file` | `${name}.ini` | single output filename within `output_path` |
| `output_encoding` | `cp1252` | INI encoding, `cp1252` or `utf-8`, for reading and writing |
...
Do not quote the executable path. `args` supplies the complete argument list.
The host neither adds nor interprets protocol options. For a Toolkit worker,
configure `--processor` and optionally `--once`. No shell string is evaluated.
...
Arguments containing spaces must be quoted. For a development installation:
...
args            = -m wiki2config_usermgr --processor --once
```

Upgrade wiki2config and its processor together to Toolkit 0.3. Remove obsolete
`operation` and `version` keys and `--session` arguments. There is no protocol
fallback. Input is named-page UTF-8 JSON, output is one complete INI document,
and the optional report is UTF-8 Markdown. The output filename and INI encoding
are application settings, not transport fields. Usermgr requires `cp1252` for
its legacy consumer. Encoding is explicit; it is never guessed.

Limits are 64 MiB input, 128 MiB output, 4 MiB report, 133 MiB encoded message,
and 64 KiB retained stderr. The timeout covers startup, conversion, and shutdown;
shutdown also has a five-second upper limit. Cleanup may extend the observed
duration. With `--once`, failed termination discards the candidate. A result
already returned by a continuing process remains valid after a later shutdown
failure, which is retained in diagnostics.

The usermgr processor uses `wiki2config_usermgr` for `-m` and builds
`wiki2config-usermgr.exe`.
