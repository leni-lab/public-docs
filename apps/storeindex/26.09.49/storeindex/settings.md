...
command = leniextract
	...
	--field keywords_raw,service,channels,dispatch_at,locations
...
```ini
[source leni]
command = .venv/Scripts/leniextract.exe --processor --field headline_raw,src_id,src_rev,keywords_raw,service,channels,dispatch_at,locations
```
...
command = leniextract --processor
    --field src_id,src_rev,dispatch_at,headline_raw,keywords_raw,service,channels,locations
```
...
defines `headline`, `keyword`, `location`, `service`, `channel`, `dispatch_at`, `src_id`,
and `src_rev`.
