...
The database must contain release times, services, channels, keywords, locations,
the source document fields `src_id` and `src_rev`, `headline`, `released_at`,
and the headline search index. Deploy with the matching Storeindex schema and
rebuild the index before starting Storeview. The former `published_at`
column has been renamed to `released_at` without a compatibility alias.
...
Draft inputs and the released filter also pause refresh. Returning to the window
...
| invalid release            | use local `YYYY-MM-DD HH:MM:SS`, without a timezone   |
