...
command = leniextract
	...
	--field keywords_raw,service,channels,dispatch_at
...
| `source <name>.mode`    | `once`               | `once` or `session`                                            |
...
```ini
[source leni]
command = .venv/Scripts/leniextract.exe --field headline_raw,src_id,src_rev,keywords_raw,service,channels,dispatch_at
```
...
Every distinct command and mode is checked before database changes, including
sources with no pending items. Once invokes the command without input and logs
its information text after exit 0. Session starts with `--session`, logs its
information response and keeps that process. A failed check stops the run.
Commands omit `--input`, `--output` and `--session`; Toolkit supplies them.
Each extractor needs to support only the configured mode.
...
Once passes unchanged source bytes using `--input -` and accepts the raw JSON
result after EOF and exit 0. Exit 3 rejects an input. Session transports the
same bytes in frames and reports rejection explicitly. Rejection allows
processing to continue, with quarantine only during incremental work. Fatal
responses, invalid options, unexpected reports and protocol failures stop the
run without quarantining the source.
...
mode = session
command = leniextract
    --field src_id,src_rev,dispatch_at,headline_raw,keywords_raw,service,channels
```

`mode` defaults to `once`, which starts one
process per document. Session mode retains one process per command and mode.
Toolkit supplies the mode flag. Normal shutdown finishes the active document,
then closes the session. The host currently requires Windows and CPython.

`index.timeout` bounds a complete once call, or session startup and each exchange separately.
Shutdown is bounded by one second or that timeout, whichever is smaller.
Toolkit owns deadlines, pipe draining and cleanup of the entire process tree.
The default limits are 64 MiB per request, 128 MiB per result and 4 MiB per report. This application accepts no reports.
Outer limit failures stop processing without quarantining the source. A timeout
discards the session; a later retry uses a fresh process.
...
defines `headline`, `keyword`, `service`, `channel`, `dispatch_at`, `src_id`,
and `src_rev`.
...
Leniextract returns `dispatch_at` from `data.transmit_at`. Missing, null, empty,
or zero values use top-level `update_at` instead. Integers and decimal strings
are accepted and stored as a required nonnegative integer in `item.dispatch_at`.
...
Add `dispatch_at` to the extractor field selection and run `rebuild` with the
...
With `channel = channels`, channels use `channel(id, value)` and `item_channel(item, channel, position)`
...
### Deploying the schema and fields
