...
operation       = wiki2config-usermgr.compile
version         = 1-draft
...
| `operation`   | required                 | exact operation name expected from the processor        |
| `version`     | required                 | exact operation version, independent of EXE version     |
...
`--processor`. Do not configure `--processor`, `--serve`, or the removed
`--output` option yourself. No shell command string is evaluated.
...
operation       = wiki2config-usermgr.compile
version         = 1-draft
args            = -m wiki2config_usermgr
```

The operation and version must match the installed processor exactly. Upgrade
wiki2config and its processor together. The previous directory protocol has no
automatic fallback. Transport limits are 64 MiB per request, 128 MiB per result,
4 MiB per report, 1 MiB per manifest, and 1024 files. stderr retention is 64 KiB
with an explicit truncation marker. These limits are fixed for this profile.
The timeout covers startup through the complete response and exit validation;
process-tree cleanup finishes before control returns and may outlast it.
