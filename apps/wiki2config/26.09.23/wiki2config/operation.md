...
The processor must return a complete file set with at least one INI file.
Subdirectories, reserved Windows filenames, metadata from the processor, and
...
`usermgr: import: users.wiki.ini updated`. Unchanged files are omitted.
...
input, protocol violations, and invalid baseline metadata wait for input changes. Fixing only the
active baseline requires a restart to trigger another attempt. Transient
filesystem, process, timeout, and Git errors retry with backoff. Settings changes stop
