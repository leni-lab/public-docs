...
`Config:User Groups` maps to `user_groups.wiki`: remove the namespace, apply
the shared name normalization, and append `.wiki`. This includes lowercase,
German umlaut expansion, and whitespace replacement with underscores. Slash,
...
For example, `* /use: permissions#templates/reader` imports that heading's
fields from `permissions.wiki`. `* /use: #templates/reader` instead addresses
the current page. Use references have no `[[...]]` brackets and are resolved
as configuration paths. The target page must be included in the conversion.
