...
For recurring operation, configure an external scheduler to run the same
command, for example once daily. Each start processes eligible files once and
exits. Use an absolute configuration path and inspect the process exit code.
See [command line](cli.md) for exit codes and shutdown behavior,
