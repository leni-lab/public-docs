...
`item.published_at` comes from the store filename and is the only time used
for chronological queries. It contains nonnegative integer Unix seconds and
...
index. Historical imports may use an approximate publication time in their
filenames, as defined by their producer.

The former `dispatch_at` column has been removed. Remove `dispatch_at` from
existing extractor commands, deploy the matching Storeview version, and run
a full `rebuild` with the new SQL schema. Initialization does not migrate an
existing database.
