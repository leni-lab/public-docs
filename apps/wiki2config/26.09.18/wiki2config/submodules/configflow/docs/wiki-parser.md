...
wiki2config selects and reads include files. The parser receives their named
...
| `node.type`                             | lowercase declared type, or `None`      |
...
Use normalized names for identity, comparisons, display, and domain output.
Original titles and field spellings stay internal for diagnostics. There are
no public label or original-title properties.
...
Effective fields include parent defaults and resolved use imports. A
...
of both operands. Original source spelling is available only in diagnostics.
