...
config_files = usermgr.wiki.ini
config_profile = usermgr
archive_files = usermgr.wiki.ini
reports = reports/*
...
| `args` | `--processor --once` | arguments passed unchanged to the executable |
| `encoding` | `auto` | original INI decoding: `auto`, `cp1252`, or `utf-8` |
| `timeout` | `30s` | startup and request limit per phase, minimum `1s` |

Format sections configure rendering and original-file decoding. The former `description` setting is
...
configured order for display, comparison, and the renderer file package. Several
