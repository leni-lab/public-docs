...
Select them with `--field` in raw CLI or processor modes. The Toolkit host
adds the mode flag, so omit `--session` from its configured command.
...
The JSON headline remains authoritative for `title`; the duplicate headline
...
`location`. Raw `markdown` preserves an explicit empty string. Malformed
...
A complete first-version EML field selection is:

```text
leniextract
  --field title,body,signal,keyword,transmit_at
...
This extension does not add columns to the storeindex database. Indexing a new
